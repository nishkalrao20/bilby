...
def lal_binary_black_hole_td(
    ...
    waveform_kwargs = dict(
        ...
        minimum_frequency=20.0, start_time=0.0, duration=4.0,
        catch_waveform_errors=False, pn_spin_order=-1, pn_tidal_order=-1,
        pn_phase_order=-1, pn_amplitude_order=0)
    waveform_kwargs.update(kwargs)
...
def _base_lal_cbc_td_waveform(
    ...
    reference_frequency = waveform_kwargs['reference_frequency']
    minimum_frequency = waveform_kwargs['minimum_frequency']
    start_time = waveform_kwargs['start_time']
    duration = waveform_kwargs['duration']
    catch_waveform_errors = waveform_kwargs['catch_waveform_errors']
    pn_spin_order = waveform_kwargs['pn_spin_order']
    pn_tidal_order = waveform_kwargs['pn_tidal_order']
    pn_phase_order = waveform_kwargs['pn_phase_order']
    ...
    time_bounds = ((time_array >= start_time) * (time_array <= start_time + duration))
    ...
    if len(hplus.data.data) > len(time_array):
        logger.debug("LALsim waveform longer than bilby's `time_array`" +
                     "({} vs {}), ".format(len(hplus.data.data), len(time_array)) +
                     "probably because padded with zeros up to the next power of two length." +
                     " Truncating lalsim array.")
        h_plus = hplus.data.data[:len(h_plus)]
        h_cross = hcross.data.data[:len(h_cross)]
