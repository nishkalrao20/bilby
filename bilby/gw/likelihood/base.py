...
from ..utils import noise_weighted_inner_product, td_noise_weighted_inner_product, td_cov_noise_weighted_inner_product, zenith_azimuth_to_ra_dec, ln_i0
...
class TDGravitationalWaveTransient(Likelihood):
    """ A time domain gravitational-wave transient likelihood object
    ...
    def calculate_snrs(self, waveform_polarizations, interferometer, return_array=True):
        ...
        """
        signal = self._compute_full_waveform_td(
            signal_polarizations=waveform_polarizations,
            interferometer=interferometer,
        )
        _mask = interferometer.time_mask
        ...
        if return_array is False:
            ...
            optimal_snr_squared_integrand = (
                normalization * np.abs(signal)**2 / interferometer.covariance_matrix
            )
            optimal_snr_squared_array = np.dot(
                optimal_snr_squared_integrand[_mask],
                self.calibration_abs_draws[interferometer.name].T
    ...
    def _calculate_noise_log_likelihood(self):
        log_l = 0
        for interferometer in self.interferometers:
            mask = interferometer.time_mask
            log_l -= td_noise_weighted_inner_product(
                interferometer.time_domain_strain[mask],
                interferometer.time_domain_strain[mask],
                interferometer.acf[mask],
                self.waveform_generator.duration) / 2
        return float(np.real(log_l))
    ...
    def log_likelihood_ratio(self):
        waveform_polarizations = \
            self.waveform_generator.time_domain_strain(self.parameters)
        if waveform_polarizations is None:
            return np.nan_to_num(-np.inf)
    ...
    def generate_time_sample_from_marginalized_likelihood(
        ...
        for ifo in self.interferometers:
            ...
            cov[:ifo_length][mask] = ifo.covariance_matrix[mask]
            d_inner_h += (signal_long * data / cov)[in_prior]
            h_inner_h += ifo.td_optimal_snr_squared(signal=signal).real
    ...
    def generate_distance_sample_from_marginalized_likelihood(
        ...
        d_inner_h, h_inner_h = self._calculate_td_inner_products(signal_polarizations)
    ...
    def _calculate_td_inner_products(self, signal_polarizations):
        d_inner_h = 0
        h_inner_h = 0
        for interferometer in self.interferometers:
            per_detector_snr = self.calculate_snrs(
    ...
    def _compute_full_waveform_td(self, signal_polarizations, interferometer):
        """
        Project the waveform polarizations against the interferometer
        response. This is useful for likelihood classes that don't
        use the full frequency array, e.g., the relative binning
        ...
        interferometer: bilby.gw.detector.Interferometer
            Interferometer to compute the response with respect to.
        """
        return interferometer.get_td_detector_response(signal_polarizations, self.parameters)
...
class TDCovGravitationalWaveTransient(Likelihood):
    """ A time domain gravitational-wave transient likelihood object
    ...
    def __init__(
        ...
        self.waveform_generator = waveform_generator
        super(TDCovGravitationalWaveTransient, self).__init__(dict())
        self.interferometers = InterferometerList(interferometers)
        self.time_marginalization = time_marginalization
        self.distance_marginalization = distance_marginalization
        self.phase_marginalization = phase_marginalization
    ...
    def calculate_snrs(self, waveform_polarizations, interferometer, return_array=True):
        ...
        d_inner_h, optimal_snr_squared = interferometer.td_cov_inner_product_optimal_snr(signal=signal)z
        complex_matched_filter_snr = d_inner_h / (optimal_snr_squared**0.5)
    ...
    def _calculate_noise_log_likelihood(self):
        ...
        for interferometer in self.interferometers:
            mask = interferometer.time_mask
            log_l -= td_cov_noise_weighted_inner_product(
                interferometer.time_domain_strain[mask],
                interferometer.time_domain_strain[mask],
                interferometer.inverse_covariance_matrix[np.ix_(mask, mask)])/2
        return float(np.real(log_l))
    ...
    def generate_time_sample_from_marginalized_likelihood(
        ...
        for ifo in self.interferometers:
            ...
            cov[:ifo_length][mask] = ifo.covariance_matrix[mask]
            d_inner_h += (signal_long * data / cov)[in_prior]
            h_inner_h += ifo.td_cov_optimal_snr_squared(signal=signal).real
    ...
    def generate_distance_sample_from_marginalized_likelihood(
        ...
        d_inner_h, h_inner_h = self._calculate_td_cov_inner_products(signal_polarizations)
    ...
    def _calculate_td_cov_inner_products(self, signal_polarizations):
        d_inner_h = 0
        h_inner_h = 0
        for interferometer in self.interferometers:
            per_detector_snr = self.calculate_snrs(
